...
describe('TextTitle Component', () => {
  ...
  it('applies a custom color when the color prop is provided', () => {
    ...
    render(<TextTitle color={customColor}>{testMessage}</TextTitle>);
  ...
  it('applies default typography styles from the theme', () => {
    ...
    expect(textComponent.props.style.fontFamily).toBe(mockTheme.typography.title.fontFamily);
    expect(textComponent.props.style.fontSize).toBe(mockTheme.typography.title.fontSize);
    expect(textComponent.props.style.lineHeight).toBe(mockTheme.typography.title.lineHeight);
  });
